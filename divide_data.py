...
def divide_images( divide_mode: str, download_path: str, title: str,train_ratio: float = 0.7, val_ratio: float = 0.15, test_ratio: float = 0.15):
        ...
        if len(img_list) == 0:
            ...
            move_files(train_files, download_path, os.path.join(download_path, 'train'), workers)
            move_files(val_files, download_path, os.path.join(download_path, 'val'), workers)
            ...
            move_files(train_files, download_path, os.path.join(download_path, 'train'), workers)
            move_files(val_files, download_path, os.path.join(download_path, 'val'), workers)
            move_files(test_files, download_path, os.path.join(download_path, 'test'), workers)
